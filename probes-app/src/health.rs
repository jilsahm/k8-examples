...
async fn health_handler(start: SystemTime, delay: u64) -> Result<impl warp::Reply, Infallible> {
    match start.elapsed().unwrap_or_default().as_secs().cmp(&delay) {
        Ordering::Greater | Ordering::Equal => Ok(warp::reply::with_status(warp::reply(), StatusCode::INTERNAL_SERVER_ERROR)),
